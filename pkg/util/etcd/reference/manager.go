...
func NewEtcdManager(OpenEOE_IP, Etcd_IP string) *EtcdManager {
	...
	// OpenEOE Master
	filename, _ := filepath.Abs("/home/nfs/openeoe/" + OpenEOE_IP + "/master/config/config")
	yaml, _ := LoadYAMLFile(filepath.Join(filename))
	cluster_name := yaml.Clusters[0].Name
	IP := SplitAny(yaml.Clusters[0].Cluster.Server, "/:")[1]

	myCluster := &MyCluster{cluster_name, IP, "2379", OpenEOE_IP, false}
	...
	// Member Clusters
	dirname, _ := filepath.Abs("/home/nfs/openeoe/" + OpenEOE_IP + "/members/join")
