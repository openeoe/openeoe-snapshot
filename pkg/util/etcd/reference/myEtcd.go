...
func NewMyETCD(myCluster *MyCluster) *MyEtcd {
	...
	openeoe_dirname, _ := filepath.Abs("/home/nfs/openeoe/" + myCluster.OpenEOEMasterIP)
	...
	if myCluster.isEtcdBackupServer {
		...
		if myCluster.IP == myCluster.OpenEOEMasterIP {
			certFilePath = filepath.Join(openeoe_dirname, "master/pki", "server.crt")
			keyFilePath = filepath.Join(openeoe_dirname, "master/pki", "server.key")
			trustedCAFilePath = filepath.Join(openeoe_dirname, "master/pki", "ca.crt")
		} else {
			certFilePath = filepath.Join(openeoe_dirname, "members/join", myCluster.IP, "pki/server.crt")
			keyFilePath = filepath.Join(openeoe_dirname, "members/join", myCluster.IP, "pki/server.key")
			trustedCAFilePath = filepath.Join(openeoe_dirname, "members/join", myCluster.IP, "pki/ca.crt")
		}
	}
	tlsInfo := transport.TLSInfo{
